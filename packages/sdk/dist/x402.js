import { verifyReceiptSignatureEIP712, verifyReceiptSignatureJWS, } from "@x402/extensions/offer-receipt";
import { isStellarNetwork } from "@x402/stellar";
function isRecord(value) {
    return typeof value === "object" && value !== null;
}
function isReceiptPayload(value) {
    if (!isRecord(value))
        return false;
...
function isCaipNetwork(value) {
    const separator = value.indexOf(":");
    return separator > 0 && separator < value.length - 1;
}
function validateSettlement(input) {
    if (input.amount <= 0n)
        throw new Error("x402 settlement amount must be positive");
    if (input.nonce < 0n)
        throw new Error("x402 settlement nonce must be non-negative");
 ...
 */
export async function parsePaymentResponse(input) {
...
export function parsedX402ToReceipt(parsed) {
    return {
        payer: parsed.payer,
        payee: parsed.payee,
        asset: parsed.asset,
