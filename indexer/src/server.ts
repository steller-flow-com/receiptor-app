...
type AdminSession = { sub: string; exp: number };
...
function requiredBigInt(body: Record<string, unknown>, key: string): bigint {
...
function configuredCredentials(): { username: string; password: string; secret: string } | null {
...
function requestCookies(req: IncomingMessage): Map<string, string> {
...
function validSession(req: IncomingMessage): boolean {
  ...
  if (!safeEqual(signature, signPayload(payload, credentials.secret))) return false;
  try {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === "POST" && path === "/receipts") {
        ...
        const result = await recordReceipt({
          ...
          contractId: requiredString(
            { contractId: process.env.RECEIPT_LEDGER_CONTRACT_ID },
            "contractId",
          ),
          merchantSecretKey,
      ...
      if (path === "/summary") {
        ...
        for (const entry of entries) statusBySource.set(entry.receiptHash, entry.status);
        let matched = 0;
        let receiptOnly = 0;
        let settlementOnly = 0;
        for (const receipt of receipts) {
          const status = statusBySource.get(receipt.receiptHash);
          if (status === "receipted") matched += 1;
          if (status === "receipt_without_settlement") receiptOnly += 1;
        }
        for (const entry of entries) {
          if (entry.status === "unreceipted_settlement" && entry.side === "debit") settlementOnly += 1;
        }
        sendJson(res, 200, {
