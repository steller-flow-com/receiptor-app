...
function ledgerRows(source: LedgerSource) {
  const base = {
...
export async function ingestEvents(
  db: DB,
  batch: { receipts: ReceiptEvent[]; settlements: SettlementEvent[] },
): Promise<void> {
  await db.transaction(async (tx) => {
    ...
    const matchedSettlementIndexes = new Set<number>();
    const receiptSources = receipts.map((receipt): LedgerSource => {
      const matchIndex = settlements.findIndex(
        (settlement, index) =>
          !matchedSettlementIndexes.has(index) &&
      ...
      if (hasSettlement) matchedSettlementIndexes.add(matchIndex);
      return {
